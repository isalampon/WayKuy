...
const Search = ({ navigation }) => {
  ...
  const fetchServices = async (category) => {
    try {
      let servicesCollection = collection(db, "Services"); // ✅ ตรวจสอบ collection(db, "Services") ถูกต้อง

      // 🔥 ใช้ query และ where ให้ถูกต้อง
      let q =
        category !== "All"
          ? query(servicesCollection, where("category", "==", category))
          : servicesCollection;
  ...
  const fetchServicesByProvinceAndCategory = useCallback(
    async (province, category) => {
      ...
      try {
        ...
        if (province !== "Near me") {
          queryConstraints.push(where("province", "==", province));
        }
        ...
        const q =
          queryConstraints.length > 0
            ? query(servicesCollection, ...queryConstraints)
            : servicesCollection;
